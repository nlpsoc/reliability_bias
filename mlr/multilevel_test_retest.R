...
# data
data = read_csv('./complete_test_retest.csv')
...
icc(model_intercept) #icc: 0.245
r.squaredGLMM(model_intercept) #conditional R2: 0.389
var.components = as.data.frame(VarCorr(model_intercept))$vcov
ICC = sum(var.components[2])/sum(var.components)
ICC

# explained variance by corpus: 0.2446187
# explained variance by model: 0.1442214
...
# scatter plot: log_n_senses vs. score - relationship seems close to zero across groups
set.seed(123)
data_std %>%
  sample_frac(0.2) %>%
...
model_full_raw = lmer('score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
...
model_full_std = lmer('score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)',
                      control = lmerControl(optimizer ="Nelder_Mead"),
                      data = data_std)

summary(model_full_std)
r.squaredGLMM(model_full_std) #R2M: 0.3261197 R2C: 0.5176527
# the residual of the intercept-only model is: 0.003848
var.components = as.data.frame(VarCorr(model_full_std))$vcov
ICC = var.components[2]/(sum(var.components)+0.003848-var.components[3])
ICC
...
# 0.01134641 (corpus) and 0.1801865 (model)

# ablation study
formula_full = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_metric = 'score ~ 1 + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_log_frequency = 'score ~ 1 + metric + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_log_frequency_squared = 'score ~ 1 + metric + log_frequency + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_log_n_senses = 'score ~ 1 + metric + log_frequency + log_frequency_squared + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_pos = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses +
                      nn_sim + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_nn_sim = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos + l2_norm + es + (1 | model) + (1 | corpus)'

formula_minus_l2_norm = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + es + (1 | model) + (1 | corpus)'

formula_minus_es = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + (1 | model) + (1 | corpus)'

formula_minus_model = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | corpus)'

formula_minus_corpus = 'score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                      nn_sim + l2_norm + es + (1 | model)'

all_formulas = list("metric" = formula_minus_metric,
                    "log_frequency" = formula_minus_log_frequency,
...
#R2M: 0.3261197 R2C: 0.5176527
do.call(rbind.data.frame, ablation_r2_ls) %>%
  mutate(var = row.names(.)) %>%
  as_tibble() %>%
  mutate(R2c_change = 0.5176527 - R2c,
         R2m_change = 0.3261197 - R2m) %>%
...
model_interact_std = lmer('score ~ 1 + metric + log_frequency + log_frequency_squared + log_n_senses + most_common_pos +
                          nn_sim + l2_norm + es +
                          log_frequency*metric + log_frequency*log_n_senses + log_frequency*nn_sim + log_frequency*l2_norm + log_frequency*es + log_frequency*most_common_pos +
                          (1 | model) + (1 | corpus)',
                          control = lmerControl(optimizer ="Nelder_Mead"),
                          data = data_std)
